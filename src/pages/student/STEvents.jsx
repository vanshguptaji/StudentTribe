...
export default function STEvents() {
  ...
  // Add missing pause states for sliders
  ...
  // Create GSAP timelines only once on mount
  useEffect(() => {
    if (mainSliderInnerRef.current && bottomSliderInnerRef.current) {
      ...
      const mainTl = gsap.timeline({ repeat: -1, paused: false });
      mainTl.fromTo(
      ...
      const bottomTl = gsap.timeline({ repeat: -1, paused: false });
      bottomTl.fromTo(
  ...
  }, [sliderImages.length]); // Only recreate on mount or if slider length changes

  // Handle pause/resume separately
  useEffect(() => {
    if (!animationRef.current) return;
    
    const { mainTl } = animationRef.current;
  ...
  useEffect(() => {
    ...
    const { bottomTl } = animationRef.current;
    if (isBottomSliderPaused) {
      bottomTl.pause();
    } else {
      bottomTl.resume();
    }
  }, [isBottomSliderPaused]);  // GSAP Animation Timeline with entrance animations
  useEffect(() => {
    ...
    >
      ...
      <div className="w-full px-0 py-8">
        ...
        >
          ...
          <div ref={mainSliderRef} className="relative mb-8 w-full overflow-hidden h-48 sm:h-64 lg:h-80 py-8" onMouseEnter={() => setIsTopSliderPaused(true)} onMouseLeave={() => setIsTopSliderPaused(false)}>
            <div
          ...
          <div
            ref={bottomSliderRef}
            className="mt-16 relative w-full overflow-hidden h-48 sm:h-64 lg:h-80 py-8"
