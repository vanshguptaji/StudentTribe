...
//         delay: 0.2,
//         onComplete: () => {
//           // Increment animation key to allow future re-triggers
...
//       entranceAnimationRef.current = entranceTl;
//     }
//   };

//   // Listen for custom events from layout
//   useEffect(() => {
//     const handleSectionAnimation = (event) => {
//       if (event.detail?.sectionName === "events") {
//         // Always trigger animation for navbar clicks
//         setTimeout(() => {
...
//     window.addEventListener("triggerSectionAnimation", handleSectionAnimation);

//     return () => {
//       window.removeEventListener("triggerSectionAnimation", handleSectionAnimation);
//     };
//   }, []);

//   // Initial setup - set elements to their final positions without animation
//   useEffect(() => {
//     if (containerRef.current && mainSliderRef.current && bottomSliderRef.current && textContentRef.current && buttonsRef.current) {
//       // Set initial positions to final state (no animation on first load)
//       gsap.set(mainSliderRef.current, { x: 0, opacity: 1 });
//       gsap.set(bottomSliderRef.current, { x: 0, opacity: 1 });
//       gsap.set(textContentRef.current, { y: 0, opacity: 1 });
...
//   // Handle slider pause/resume on hover
//   useEffect(() => {
//     if (animationRef.current) {
//       if (isTopSliderPaused) {
...
//           {/* Top Image Slider - Continuous scroll left to right */}
//           <div ref={mainSliderRef} className="relative mb-8 w-full overflow-hidden bg-rose-100 py-16 rounded-2xl">
//             <div
//               ref={mainSliderInnerRef}
//               className="flex items-center space-x-2 sm:space-x-4 w-fit"
//             >
...
//                   className="relative flex-shrink-0 flex justify-center"
//                   style={{ width: `calc(100vw / ${sliderImages.length})` }}
//                   onMouseEnter={() => setIsTopSliderPaused(true)}
//                   onMouseLeave={() => setIsTopSliderPaused(false)}
//                 >
//                   <img
//                     src={image}
//                     alt={`Event ${(index % sliderImages.length) + 1}`}
//                     className="w-48 h-32 sm:w-64 sm:h-48 lg:w-80 lg:h-60 rounded-lg object-cover opacity-80 transition-all duration-500 mx-auto z-10 cursor-pointer hover:opacity-100 hover:scale-150 hover:shadow-2xl hover:z-20"
//                   />
//                   <div className="absolute inset-0 bg-gradient-to-t from-black/20 to-transparent rounded-lg pointer-events-none"></div>
//                 </div>
//               ))}
...
//           <div
//             ref={bottomSliderRef}
//             className="mt-16 relative w-full overflow-hidden bg-rose-100 py-16 rounded-2xl "
//           >
//             <div
//               ref={bottomSliderInnerRef}
//               className="flex items-center space-x-2 sm:space-x-4 w-fit"
...
//                   className="relative flex-shrink-0 flex justify-center"
//                   style={{ width: `calc(100vw / ${sliderImages.length})` }}
//                   onMouseEnter={() => setIsBottomSliderPaused(true)}
//                   onMouseLeave={() => setIsBottomSliderPaused(false)}
//                 >
//                   <img
//                     src={image}
//                     alt={`Bottom event ${(index % sliderImages.length) + 1}`}
